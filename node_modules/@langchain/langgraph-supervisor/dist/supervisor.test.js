...
describe("Supervisor", () => {
    it("should be drawable as subgraph", async () => {
        const getSubGraph = async () => {
            ...
            const webSearch = tool(async (_args) => {
    ...
    it("should work with multiple layers of supervision arranged as subgraphs", async () => {
        const RECORDED_MESSAGES_FILE = path.resolve(path.join(__dirname, "..", "resources", "recorded_messages_multiple_layers_of_supervision.json"));
        if (!existsSync(RECORDED_MESSAGES_FILE)) {
            throw new Error("Recorded messages file not found!");
        }
        const recordedMessages = JSON.parse(readFileSync(RECORDED_MESSAGES_FILE, "utf8"));
        ...
        const writingTool = tool(async (args) => {
        ...
        const publishingTool = tool(async (args) => {
